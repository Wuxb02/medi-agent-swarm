...
const flattenedNodes = computed(() =>
  props.treeData ? flatten(props.treeData, 0) : []
)

function dotColor(item: FlatNode): string {
  if (item.status === 'error') return 'bg-red-400'
  const map: Record<string, string> = {
    trace: 'bg-slate-400', stage: 'bg-blue-400',
    agent: 'bg-green-400', iteration: 'bg-purple-400',
    llm: 'bg-orange-400', tool: 'bg-cyan-400',
  }
