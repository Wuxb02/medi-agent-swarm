...
async def confirm_pending_item(body: PendingConfirmRequest):
    """确认待确认条目：从暂存区移入已确认信息"""
    # confirm_pending 内部会同时：从 PENDING.md 删除 + 写入 CONFIRMED.md
    success = profile_manager.confirm_pending(body.key, body.value)
    if not success:
...
async def dismiss_pending_item(body: PendingConfirmRequest):
    """丢弃待确认条目"""
    profile_manager.dismiss_pending(body.key, body.value)
    return {"status": "ok", "key": body.key, "value": body.value}
...
@router.put("/records")
async def update_medical_records(body: MedicalRecordsUpdate):
    """更新病史记录（全量替换）"""
    from memory.personal_profile import MedicalRecord
    records = [
        MedicalRecord(
            date=r.date, description=r.description,
            symptoms=r.symptoms, duration=r.duration,
    ...
    profile_manager.save_records(records)
