...
VISION_PROMPT = """请仔细分析这张医学相关的图片，用中文描述其内容。重点关注：

1. 如果是**检验报告/化验单**：提取所有项目名称、数值、参考范围，用 ↑↓ 标注异常项
2. 如果是**皮肤照片**：描述皮损的颜色、形态、分布、范围
3. 如果是**处方单/药品**：识别药品名称、剂量、用法
4. 如果是**影像资料**(CT/MRI/X光)：描述可见的结构异常
5. 其他类型：尽可能提取图中的文字信息和视觉特征

请用客观、专业的语言描述，不要给出诊断结论（诊断由医生完成）。"""
...
class ImageAnalyzer:
    ...
    async def analyze(self, image_paths: List[str], user_question: str) -> str:
        ...
        enhanced = (
            f"[图片分析结果]\n"
            f"以下是对用户上传的 {len(descriptions)} 张图片的 AI 分析描述：\n\n"
            f"{image_context}\n\n"
            f"---\n"
