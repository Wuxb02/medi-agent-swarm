...
上传/更新接口支持设置可选的 `effective_at` / `expires_at` 时间范围。文档到期后，由 `DataLifecycleService` 每日作业将 `active` 标记为 `expired`（非 active），读取侧仍以 `_is_effective()` 实时判断兜底，保证定时作业未跑到的窗口期内也不放行过期内容。`expired` 是「现行但已失效」的中间态，保留物理数据与版本链，待新版本顶替时转为 `archived`；管理端文档列表会展示各文档的生效/失效时间与过期状态。
